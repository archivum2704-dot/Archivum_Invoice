...
>
> Última actualización: **15 de septiembre de 2026 (raya blanca bajo el teclado en los modales de pantalla completa del móvil)**
...
| **Cargar en Vercel el email/fecha/lugar de la declaración responsable firmada** | Nosotros | Firmada el 2 de septiembre de 2026 por las dos socias (ver abajo) — falta cargar `VERIFACTU_PRODUCER_EMAIL`, `VERIFACTU_DECLARATION_PLACE`, `VERIFACTU_DECLARATION_DATE` y `VERIFACTU_DECLARATION_ISSUED=true` en Vercel y redesplegar para que `/declaracion-responsable` deje de mostrar "no emitida" |
...
| **Confirmar que la AEAT acepta un envío tras el arreglo del 3 de septiembre** | | La primera prueba real contra preproducción (3 sept, certificado de pruebas persona jurídica `A39200019`) reveló que `Encadenamiento.RegistroAnterior` solo llevaba la huella — la AEAT rechazó el envío entero con SOAP Fault Codigo[4102] ("Falta informar campo obligatorio: NumSerieFactura"), un fallo de fondo desde que existe el encadenamiento (6 de agosto), nunca detectado porque ningún chequeo comprobaba ese bloque. Corregido y desplegado; reparadas en la base de datos las 29 facturas ya emitidas con el mismo defecto. **Falta confirmar que un reenvío ya lo acepta** |
...
| `VERIFACTU_PRODUCER_EMAIL` | ⚠️ pendiente de cargar | Declaración responsable, apartado 2.a. Valor a usar: `facturas@archivum.es` |
| `VERIFACTU_DECLARATION_PLACE` / `_DATE` / `_ISSUED` | ⚠️ pendiente de cargar | Idem. La declaración ya está firmada (2 de septiembre de 2026, ver abajo) — valores: `Aranda de Duero` / `2 de septiembre de 2026` / `true` |
...
| Fecha | Qué |
|---|---|
| 15 sept | **Corregido: raya blanca debajo del contenido al abrir el teclado en el móvil, que se quedaba aunque se cerrara.** Reportado en Pedido, pero afectaba a los 4 modales de pantalla completa que hay (Pedido y Factura en `presupuestos.tsx`/`facturacion.tsx`, y los dos de administración en `ajustes.tsx`) porque todos comparten `KeyboardModal` — un modal no transparente de React Native dibuja sobre el fondo de ventana por defecto de Android (blanco, incluso en modo oscuro) en cualquier zona que nadie más pinte. Cada pantalla cubre su `SafeAreaView` con `edges={["top"]}` a propósito, dejando fuera el margen inferior porque ya lo reserva el propio `KeyboardModal` con `paddingBottom: insets.bottom` — y esa franja no tenía color de fondo. Se quedaba invisible hasta que algo forzaba un repintado ahí, y abrir o cerrar el teclado lo hacía. Arreglado en la raíz, en `KeyboardModal.tsx`: se pinta `C.bg` (color de fondo del tema, claro u oscuro) detrás de todo el modal en vez de perseguir cada disparador de repintado — se salta para los modales `transparent` (selectores, hojas inferiores), que ya pintan su propia superposición. Al vivir en un único componente compartido, corrige las 4 pantallas de una vez, que es justo la regla que este archivo pide seguir para no duplicar UI |
| 15 sept | **Corregido: "unauthorized" en crudo al crear un pedido, en la web.** Un cliente reportó ver ese código literal en rojo en el panel de "Nuevo pedido" en vez de un mensaje. Causa: `POST /api/quotes` devuelve 401 cuando `getUser()` no encuentra sesión válida en las cookies, y `handleSave()` en `presupuestos-view.tsx` mostraba `json.error` tal cual cuando no había `detail` — mismo patrón sin traducir en varias vistas más (`albaranes-view.tsx`, `facturacion-view.tsx`...), pero el arreglo se acotó aquí, que es donde se reportó. La sesión puede caducar mientras se rellena un pedido largo porque el middleware — que es quien la refresca en cada navegación — no toca `/api` a propósito (ver más abajo, "el middleware no debe redirigir `/api`"), así que nada la refresca hasta la siguiente carga de página. `handleSave` ahora, ante un 401, intenta `supabase.auth.refreshSession()` y reintenta el envío una vez antes de rendirse — así no se pierde un pedido ya escrito por una caducidad que se resuelve sola. Si sigue fallando, mensaje claro en vez del código; añadido además un mapa `QUOTE_ERROR_MESSAGES` para el resto de códigos que puede devolver esa ruta |
| 15 sept | **Buscador de productos en el móvil, al añadir una línea a un pedido o una factura.** Otro aviso del mismo cliente: sin forma de localizar un producto concreto. La fila de "Descripción" en Presupuestos y Facturación (móvil) listaba **todos** los productos del inventario como chips en un scroll horizontal — sin filtro, inservible en cuanto el catálogo crece (la web ya tenía esto resuelto con `ProductPicker`, un combobox con búsqueda, en `facturacion-view.tsx`). Nuevo `apps/mobile/components/ProductPickerModal.tsx`, compartido por ambas pantallas para que no vuelva a divergir: hoja inferior con buscador y lista filtrada por nombre, mismo patrón visual que el selector de cliente que ya existía en esas mismas pantallas. La fila de chips se sustituye por un botón que abre la hoja y muestra el producto ya vinculado a la línea, si lo hay |
| 3 sept | **Certificado VERI\*FACTU también desde el móvil.** Hasta ahora solo se podía subir desde `/configuracion/verifactu` en la web. Nuevo `VerifactuCertModal` en Ajustes (`apps/mobile/app/(app)/ajustes.tsx`, solo admins), contra el mismo endpoint `/api/verifactu/certificate` sin ningún cambio en el servidor: selector de archivo con `expo-document-picker` (ya usado en Subir, así que no hace falta build nativo nuevo — `package.json` no se ha tocado), lectura en base64 y el mismo estado/subida/borrado que ya existía en la web |
| 3 sept | **Listado imprimible de albaranes/pedidos, para puntear a mano antes de facturar.** Petición del cliente: no cambia el flujo de facturación (sigue siendo un albarán = una factura, uno a uno desde `AlbaranesView`), solo añade un modo de selección múltiple ("Seleccionar para imprimir") que genera un PDF con casilla en blanco por fila — `lib/quote-list-pdf.ts` (nuevo) + `POST /api/quotes/print-list` (nuevo). El total agregado del pie solo aparece cuando todas las filas seleccionadas comparten moneda, para no sumar EUR con USD/GBP. Ojo si se vuelve a tocar `quote-list-pdf.ts`: la función `clean()` que sanea el texto antes de `drawText` estaba descartando el símbolo `€` (fuera de Latin-1, \x00-\xFF) — WinAnsiEncoding, la que usan las fuentes estándar de pdf-lib, sí lo soporta, así que se dejó como excepción explícita en el regex |
| 3 sept | **Corregido: la verificación en dos pasos se quedaba cargando sin avanzar, en el móvil.** Ningún punto de la cadena de login (`signInEmpresa`/`signInUsuario`/`verifyMfaCode` en `auth-context.tsx`, `handleLogin` en `login.tsx`) tenía try/catch — un fallo de red durante el MFA dejaba el spinner girando para siempre en vez de mostrar un error. Añadidos con mensajes en español; `loadProfile` ya no puede tirar abajo el login si falla después de autenticar. Mismo patrón aplicado en `/auth/mfa` (web), con una clave de traducción nueva (`mfa.challenge.connectionError`) en vez de reutilizar el texto de "código incorrecto" |
| 3 sept | **`Encadenamiento.RegistroAnterior` incompleto — la primera prueba real contra la AEAT lo detectó al momento.** Un cliente no podía emitir facturas con líneas exentas de IVA (`doIssue()` en `facturacion-view.tsx`, a diferencia de `saveInvoiceDraft` y de Presupuestos, nunca mandaba `exemptionCause` al servidor — arreglado). Al probar el arreglo contra preproducción de la AEAT con el certificado de pruebas, salió un fallo mayor: el envío entero rechazado con SOAP Fault Codigo[4102] ("Falta informar campo obligatorio: NumSerieFactura"). La causa: `Encadenamiento.RegistroAnterior` solo guardaba la `Huella` del registro anterior — el esquema exige también `IDEmisorFactura`, `NumSerieFactura` y `FechaExpedicionFactura`. Bug de fondo desde el 6 de agosto (cuando se implementó el encadenamiento), invisible hasta hoy porque nadie había enviado nada de verdad a la AEAT y ningún chequeo de `verifactu-check.ts` comprobaba ese bloque (ya se añadieron). `readChainHead()` ya leía `full_number`/`issue_date` del eslabón anterior — la función SQL `verifactu_chain_head` los devuelve — pero se descartaban; ahora se propagan por `ChainLink` → `buildRegistroAlta`/`buildRegistroAnulacion` → XML. Reparadas en la base de datos, vía SQL directo (no toca `huella` ni ninguna columna que la inmutabilidad proteja), las 29 facturas ya emitidas con el mismo defecto, en las organizaciones de prueba y en la de producción. **Pendiente confirmar que un reenvío ya lo acepta la AEAT** |
| 2 sept | **Términos y condiciones y Política de privacidad con texto legal real, recibido del abogado.** `/terminos` y `/privacidad` dejan de usar `LegalPlaceholder` (el índice provisional) y pasan a un nuevo componente `components/legal-document.tsx` (renderiza artículos/listas/citas a partir de un array de contenido, en vez de 700+ líneas de JSX repetido) con el texto completo entregado por el cliente. El único email de contacto en ambos documentos es `facturas@archivum.es` (decisión explícita del cliente, aunque es el remitente automático de facturas por Resend, no un buzón pensado para RGPD — revisar si conviene separarlo más adelante). Privacidad fechada a 1 de septiembre de 2026, igual que Términos. **`/cookies` sigue siendo el placeholder** — es la única de las tres que no ha llegado todavía; ver el inventario técnico de cookies (Supabase/GA/Vercel Analytics/Sentry/Stripe/2FA) hecho en la sesión anterior para cuando llegue el texto. Además llegó firmada (2 de septiembre de 2026, Virginia Ocaña del Cura y María Victoria Peña Ruiz) la **declaración responsable del sistema informático** exigida por el art. 13.2 del RD 1007/2023 — un documento en prosa distinto del que genera `lib/declaracion-responsable.ts` (que sigue literalmente la estructura oficial 1.a-1.l de la AEAT, a propósito, para que quien lo revise vea el formulario que espera la Agencia y no una paráfrasis). No se ha tocado ese generador ni se ha subido el documento firmado al repo — solo aporta los datos que faltaban (email, lugar, fecha) para poder emitirlo: falta cargar `VERIFACTU_PRODUCER_EMAIL=facturas@archivum.es`, `VERIFACTU_DECLARATION_PLACE=Aranda de Duero`, `VERIFACTU_DECLARATION_DATE=2 de septiembre de 2026` y `VERIFACTU_DECLARATION_ISSUED=true` en Vercel y redesplegar; hasta entonces `/declaracion-responsable` sigue mostrando "no emitida", correctamente |
| 30 ago | **Aviso de stock mínimo también al facturar, no solo en Inventario.** Antes el stock se descontaba en silencio al emitir una factura, sin avisar de nada. Ahora, justo antes de emitir — Albaranes, el detalle de un albarán y Facturación directa, en **web y móvil**, los tres sitios desde los que se puede facturar en cada app — se comprueba si alguna línea con producto de stock controlado dejaría el stock igual o por debajo de su mínimo; si es así, aparece un aviso con el detalle por producto (stock actual, cantidad de la factura, stock resultante, mínimo) y hay que confirmar explícitamente "Emitir de todas formas" para seguir, o cancelar. La regla de qué cuenta como stock bajo vive en un único sitio, `apps/web/lib/stock.ts` (`isLowStock` + `getStockWarnings`), espejado en `apps/mobile/lib/stock.ts` — `inventario-view.tsx` en la web ahora importa `isLowStock` de ahí en vez de definirlo por su cuenta, para que no vuelva a divergir entre pantallas. En el móvil el aviso usa `Alert.alert` nativo (`lib/stock-warning-alert.ts`), no un modal a medida, para no romper el patrón ya usado en esas pantallas |
