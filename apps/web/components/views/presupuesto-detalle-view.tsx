...
import { ArrowLeft, Printer, Download, Pencil, ArrowRight, Loader2 } from "lucide-react"
...
export function PresupuestoDetalleView({ id }: { id: string }) {
  ...
  useEffect(() => {
    fetchQuoteWithLines(id).then(data => {
      if (data) { setQuote(data.quote); setLines(data.lines) }
      setLoading(false)
    })
  }, [id])
  ...
  const handleConvert = async () => {
    if (!quote) return
    if (!confirm(`¿Convertir el presupuesto ${quote.full_number ?? ""} en factura?`)) return
    setConverting(true)
    ...
    if (!res.ok) { alert(json.detail ?? json.error ?? "No se pudo convertir."); setConverting(false); return }
  ...
  if (!quote) return (
    <div className="p-8 text-center">
      <p className="text-sm text-muted-foreground mb-3">Presupuesto no encontrado.</p>
  ...
  return (
    <div className="p-6 sm:p-8 max-w-3xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 print:hidden">
        <Link href="/presupuestos" className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors">
          <ArrowLeft className="w-4 h-4" /> Presupuestos
        </Link>
          ...
          {quote.status !== "converted" && (
            <>
              ...
              <button onClick={handleConvert} disabled={converting} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold bg-primary text-primary-foreground rounded-xl hover:bg-primary/90 disabled:opacity-50 transition-colors">
                {converting ? <Loader2 className="w-4 h-4 animate-spin" /> : <ArrowRight className="w-4 h-4" />} Convertir a factura
              </button>
            </>
          )}
      ...
      <div className="bg-card border border-border rounded-2xl p-8 print:border-0 print:shadow-none">
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="text-right">
            <p className="text-xs font-semibold tracking-wide text-muted-foreground">PRESUPUESTO</p>
        ...
        <div className="mb-6">
          <p className="text-xs font-semibold tracking-wide text-muted-foreground mb-1">PRESUPUESTO PARA</p>
        ...
        <p className="text-[10px] text-muted-foreground/70 mt-4">Este documento es un presupuesto y no tiene validez como factura.</p>
      </div>
    </div>
  )
}
