...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const admin = getAdminClient()
  const { error } = await admin.from('organizations').update(update).eq('id', params.id)
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  await Promise.all([
    admin.from('documents').delete().eq('organization_id', params.id),
    admin.from('companies').delete().eq('organization_id', params.id),
    admin.from('organization_members').delete().eq('organization_id', params.id),
  ])
