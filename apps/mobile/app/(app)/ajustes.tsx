...
function StatusIcon({ status, C }: { status: string; C: Colors }) {
  ...
  if (status === "trialing") return <Clock        size={14} color={C.blue} />;
  if (status === "past_due") return <AlertTriangle size={14} color={C.yellow} />;
  return <XCircle size={14} color={C.muted} />;
}

function statusColor(s: string, C: Colors) {
  if (s === "active")   return C.green;
  if (s === "trialing") return C.blue;
  if (s === "past_due") return C.yellow;
  return C.muted;
}
...
function UsageBar({ value, max, C }: { value: number; max: number; C: Colors }) {
  ...
  return (
    <View style={{ height: 5, backgroundColor: C.border, borderRadius: 3, marginTop: 6, overflow: "hidden" }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        ...
        <Card C={C}>
          <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", padding: 14, borderBottomWidth: 1, borderBottomColor: C.border }}>
            ...
            {plan ? (
              <View style={{ flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: statusColor(plan.subscription_status, C) + "18", paddingHorizontal: 8, paddingVertical: 4, borderRadius: 20 }}>
                <StatusIcon status={plan.subscription_status} C={C} />
                <Text style={{ fontSize: 12, fontWeight: "600", color: statusColor(plan.subscription_status, C) }}>
              ...
              <View style={{ padding: 14, borderBottomWidth: 1, borderBottomColor: C.border }}>
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 6 }}>
                    ...
                    <Text style={{ fontSize: 13, color: C.muted }}>{t("ajustes.plan.docsAvailable")}</Text>
                  </View>
        ...
        <Card C={C}>
          <View style={{ padding: 14, borderBottomWidth: 1, borderBottomColor: C.border }}>
            ...
            <View style={{ flexDirection: "row", backgroundColor: C.segmentBg, borderRadius: 8, padding: 2 }}>
              {LANGS.map((l) => {
          ...
          <View style={{ padding: 14, borderBottomWidth: 1, borderBottomColor: C.border }}>
            ...
            <View style={{ flexDirection: "row", backgroundColor: C.segmentBg, borderRadius: 8, padding: 2, gap: 2 }}>
              {THEMES.map((th) => {
        ...
        <Card C={C}>
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: 14, color: C.red, fontWeight: "600" }}>{t("ajustes.cancelSub.title")}</Text>
